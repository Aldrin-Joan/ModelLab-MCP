...
class TenantOrm(Base):
    ...
    principals: Mapped[list["PrincipalOrm"]] = relationship(back_populates="tenant", cascade="all, delete-orphan")
    projects: Mapped[list["ProjectOrm"]] = relationship(back_populates="tenant", cascade="all, delete-orphan")
...
class ProjectOrm(Base):
    ...
    datasets: Mapped[list["DatasetOrm"]] = relationship(back_populates="project", cascade="all, delete-orphan")
    experiments: Mapped[list["ExperimentOrm"]] = relationship(back_populates="project", cascade="all, delete-orphan")
...
class ModelOrm(Base):
    ...
    versions: Mapped[list["ModelVersionOrm"]] = relationship(back_populates="model", cascade="all, delete-orphan")

    __table_args__ = (
        Index("ix_models_tenant_created", "tenant_id", "created_at"),
    )
...
class ModelVersionOrm(Base):
    ...
    hyperparameters_schema: Mapped[list[dict[str, Any]]] = mapped_column(JSON_TYPE, default=list, nullable=False)
...
class DatasetOrm(Base):
    ...
    versions: Mapped[list["DatasetVersionOrm"]] = relationship(back_populates="dataset", cascade="all, delete-orphan")
...
class ExperimentOrm(Base):
    ...
    runs: Mapped[list["ExperimentRunOrm"]] = relationship(back_populates="experiment", cascade="all, delete-orphan")
    metrics: Mapped[list["MetricOrm"]] = relationship(back_populates="experiment", cascade="all, delete-orphan")
    artifacts: Mapped[list["ArtifactOrm"]] = relationship(back_populates="experiment", cascade="all, delete-orphan")
    analysis: Mapped["AnalysisRunOrm | None"] = relationship(back_populates="experiment", uselist=False, cascade="all, delete-orphan")
...
class ExperimentRunOrm(Base):
    ...
    metrics: Mapped[list["MetricOrm"]] = relationship(back_populates="run", cascade="all, delete-orphan")
...
class MetricOrm(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=generate_uuid7)
    experiment_id: Mapped[str] = mapped_column(ForeignKey("experiments.id", ondelete="CASCADE"), nullable=False)
    run_id: Mapped[str] = mapped_column(ForeignKey("experiment_runs.id", ondelete="CASCADE"), nullable=False)
    metric_name: Mapped[str] = mapped_column(String(64), nullable=False)
    metric_value: Mapped[float] = mapped_column(Float, nullable=False)
    step: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    split: Mapped[str] = mapped_column(String(32), default="validation", nullable=False)  # train, validation, test
    created_at: Mapped[datetime] = mapped_column(default=utc_now, nullable=False)
...
class AnalysisRunOrm(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=generate_uuid7)
    experiment_id: Mapped[str] = mapped_column(ForeignKey("experiments.id", ondelete="CASCADE"), unique=True, nullable=False)
    tenant_id: Mapped[str] = mapped_column(String(36), nullable=False)
...
class ArtifactOrm(Base):
    ...
    tenant_id: Mapped[str] = mapped_column(String(36), nullable=False)
    experiment_id: Mapped[str] = mapped_column(ForeignKey("experiments.id", ondelete="CASCADE"), nullable=False)
    artifact_type: Mapped[str] = mapped_column(String(64), nullable=False)  # model, predictions, metrics_report, etc.
    storage_uri: Mapped[str] = mapped_column(String(512), nullable=False)
    size_bytes: Mapped[int] = mapped_column(BigInteger, nullable=False)
    content_hash: Mapped[str] = mapped_column(String(128), nullable=False)
...
class OutboxEventOrm(Base):
    ...
    status: Mapped[str] = mapped_column(String(32), default="PENDING", nullable=False)  # PENDING, DELIVERED, FAILED
