...
class OverfittingDetector:
    ...
    def evaluate(
        ...
        for k in common_keys:
            ...
            val_v = val_metrics[k]
            if (
                isinstance(tr_v, (int, float))
                and isinstance(val_v, (int, float))
                and k in ("accuracy", "roc_auc", "pr_auc", "f1", "f1_macro", "r2")
            ):
                gap = tr_v - val_v
        ...
        return {
            ...
            "severity": "HIGH"
            if any(g > 0.25 for g in gap_details.values())
            else ("MEDIUM" if overfitting_detected else "LOW"),
            "observation": (
                f"Validation performance lags training metrics by {max(gap_details.values()):.2%}, indicating model memorization."
                if overfitting_detected and gap_details
                else "Generalization gap is within acceptable operational tolerance."
            ),
        }
    ...
    def evaluate(
        ...
        for k in ("roc_auc", "accuracy", "f1"):
            if val_metrics.get(k) is not None and val_metrics[k] >= 0.999:
                signals.append(
                    f"Metric '{k}' is suspiciously perfect ({val_metrics[k]:.4f}) on validation split."
                )
                suspicious = True
        ...
        for k in ("accuracy", "f1", "roc_auc"):
            if k in train_metrics and k in val_metrics and val_metrics[k] - train_metrics[k] > 0.20:
                signals.append(
                    f"Validation '{k}' ({val_metrics[k]:.2f}) significantly exceeds training ({train_metrics[k]:.2f})."
                )
                suspicious = True
